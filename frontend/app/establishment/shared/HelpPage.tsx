...
import { useCampusContext } from "../useEstablishment";
...
export default function HelpPage({ role }: { role: RoleSlug }) {
  ...
  return <Shell role={role} activeSlug="help" displayName={context?.displayName} institutionName={context?.institutionName} note={error ?? null}>
    <header className={styles.head}><div><small>ESPACE {role.toUpperCase()}</small><h1>Aide & support</h1><p>Choisissez le parcours qui correspond à votre demande.</p></div><Link className={styles.back} href={`/establishment/${role}`}>← Accueil</Link></header>
