...
export default function AssignmentsPage({ role }: { role: Extract<RoleSlug, "admin" | "pedagogy"> }) {
  ...
  return <Shell role={role} activeSlug="assessments" displayName={context?.displayName} institutionName={context?.institutionName} note={contextError ?? null}>
    <header className={styles.head}><div><small>ESPACE {role.toUpperCase()}</small><h1>Évaluations</h1><p>Devoirs enregistrés dans les classes de votre périmètre.</p></div><Link href={`/establishment/${role}`} className={styles.back}>← Accueil</Link></header>
