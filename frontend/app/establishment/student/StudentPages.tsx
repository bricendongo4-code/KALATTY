...
const STUDENT_NAV: Array<{ section: Section; label: string; icon: "calendar" | "book" | "users" | "award" | "file" }> = [
  { section: "schedule", label: "Planning", icon: "calendar" },
  { section: "subjects", label: "Cours", icon: "book" },
  { section: "class", label: "Classe", icon: "users" },
  { section: "results", label: "Résultats", icon: "award" },
  { section: "documents", label: "Documents", icon: "file" },
];
...
export default function StudentPages({ section }: { section: Section }) {
  ...
  return <Shell role="student" activeSlug={section} displayName={context?.displayName} institutionName={context?.institutionName} note={contextError ?? null}>
    ...
    {loading || contextLoading ? <div className={styles.state}>Chargement des données de votre établissement…</div> : error || contextError ? <div className={styles.state} role="alert">{error ?? contextError}</div> : !data ? null : <>
      <nav className={styles.sectionNav} aria-label="Navigation de l’espace étudiant">{STUDENT_NAV.map((item) => <Link key={item.section} href={`/establishment/student/${item.section}`} className={section === item.section ? styles.sectionNavActive : ""}><Icon name={item.icon} /><span>{item.label}</span></Link>)}</nav>
      <section className={styles.overview} aria-label="Résumé de votre scolarité"><article><strong>{data.rooms[0]?.name ?? "Non affecté"}</strong><span>Ma classe</span></article><article><strong>{data.courses.length}</strong><span>Cours inclus</span></article><article><strong>{data.schedule.length}</strong><span>Créneaux semaine</span></article><article><strong>{data.grades.length}</strong><span>Résultats publiés</span></article></section>
      ...
      {section === "subjects" ? data.courses.length ? <div className={styles.grid}>{data.courses.map((course) => <article className={styles.tile} key={course.id}><span className={styles.icon}><Icon name="book" /></span><small>{course.roomName}</small><span className={styles.included}>Inclus par votre établissement</span><h2>{course.title}</h2><p>{course.description || "Formation attribuée à votre classe."}</p><Link href={`/learn/courses/${course.id}`}>Continuer le cours →</Link></article>)}</div> : <Empty text="Aucun cours en ligne n’est encore attribué à votre classe." /> : null}
