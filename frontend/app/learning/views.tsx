...
function CourseRow({ course, trainer = false }: { course: CourseView; trainer?: boolean }) {
  return <article className={styles.courseRow}>
    ...
    <Link href={trainer ? `/learning/formateur/formations/${course.id ?? "builder"}` : course.id ? `/learning/apprenant/formations/${course.id}` : "/learning/apprenant/formations"} className={styles.smallButton}>{trainer ? "Gérer" : "Continuer"}</Link>
  </article>;
}
...
export function LearnerHome({ data }: { data?: LearningDashboardData }) {
  ...
  return <>
    ...
    {resume ? <section className={styles.resumeCard}>
      ...
      <Link href={resume.id ? `/learning/apprenant/formations/${resume.id}` : "/learning/apprenant/formations"} className={styles.primaryButton}><Icon name="play" /> Reprendre</Link>
    </section> : <section className={styles.emptyLearning}><Icon name="book" /><div><h2>Aucune formation en cours</h2><p>Explorez le catalogue pour commencer votre premier parcours.</p></div><Link href="/learning/apprenant/explorer" className={styles.primaryButton}>Explorer</Link></section>}
    <div className={styles.sectionHead}><h2>Mes formations en cours</h2><Link href="/learning/apprenant/formations">Voir toutes</Link></div>
    ...
    <div className={styles.twoColumns}>
      ...
      <section className={styles.panel}><div className={styles.sectionHead}><h2>Recommandations pour vous</h2><Link href="/learning/apprenant/explorer">Explorer</Link></div><div className={styles.recommendations}><span>Intelligence artificielle<small>1 164 apprenants</small></span><span>Entrepreneuriat<small>980 apprenants</small></span><span>Communication efficace<small>760 apprenants</small></span></div></section>
    </div>
  </>;
}
...
export function TrainerHome({ data }: { data?: LearningDashboardData }) {
  ...
  return <>
    ...
    <section className={styles.studioCallout}><div><Icon name="video" /><span><strong>Créez votre prochain cours avec Kalatty Studio</strong><small>Enregistrez, montez, ajoutez des interactions et publiez facilement.</small></span></div><Link href="/learning/formateur/studio" className={styles.primaryButton}>Ouvrir le Studio →</Link></section>
  </>;
}
