...
type Course = { id: string; title: string; description: string; shortDescription?: string; objectives?: string; prerequisites?: string; level?: string; priceFcfa: number; teacherId?: string; teacherName?: string; teacherExpertise?: string; progressPercentage: number; enrolled: boolean; ownerPreview?: boolean; institutionAccess?: boolean; modules: Array<{ id: string; title: string; description: string; lessons: Lesson[]; exercises: Array<{ id: string; title: string; instructions: string }> }> };
...
export default function CoursePlayerPage({ courseId, initialLessonId }: { courseId: string; initialLessonId?: string }) {
  ...
  if (!course.enrolled && !course.institutionAccess && !course.ownerPreview) return <>
    ...
    <section className={styles.courseAccessHero}><div><span className={styles.eyebrow}>Formation en ligne</span><h1>{course.title}</h1><p>{course.shortDescription || course.description}</p><small>Par {course.teacherId ? <Link href={`/learn/trainers/${course.teacherId}`}>{course.teacherName ?? "Formateur Kalatty"}</Link> : course.teacherName ?? "Formateur Kalatty"} · {lessons.length} leçon(s){course.level ? ` · ${course.level}` : ""}</small></div><aside><strong>{course.priceFcfa > 0 ? `${new Intl.NumberFormat("fr-FR").format(course.priceFcfa)} FCFA` : "Gratuit"}</strong><button disabled={buying} className={styles.primaryButton} onClick={activateAccess}>{buying ? "Traitement…" : course.priceFcfa > 0 ? "Acheter la formation" : "S’inscrire gratuitement"}</button>{accessMessage ? <p>{accessMessage}</p> : null}<small>L’accès est activé uniquement après confirmation du serveur.</small></aside></section>
    <div className={styles.twoColumns}><section className={styles.panel}><h2>À propos de cette formation</h2><p className={styles.courseLongCopy}>{course.description}</p>{course.objectives ? <><h3>Objectifs</h3><p className={styles.courseLongCopy}>{course.objectives}</p></> : null}{course.prerequisites ? <><h3>Prérequis</h3><p className={styles.courseLongCopy}>{course.prerequisites}</p></> : null}</section><section className={styles.panel}><h2>Programme</h2><div className={styles.publicProgram}>{course.modules.map((module) => <div key={module.id}><strong>{module.title}</strong><span>{module.lessons.length} leçon(s)</span></div>)}</div></section></div>
  </>;
