...
const DETAILS: Record<LearningRole, Record<string, { title: string; text: string; tabs: string[] }>> = {
  apprenant: {
    ...
    messages: { title: "Notifications", text: "Informations et alertes de votre compte.", tabs: ["Toutes", "Non lues"] },
...
export default function LearningSectionPage({ role, slug }: { role: LearningRole; slug: string }) {
  ...
  return <>
    ...
    {["explorer", "formations", "messages", "notifications"].includes(slug) ? <nav className={styles.tabs}>{details.tabs.filter((tab) => slug !== "formations" || role !== "formateur" || tab !== "Archivées").map((tab) => <button key={tab} type="button" className={activeTab === tab ? styles.tabActive : ""} onClick={() => setActiveTab(tab)}>{tab}</button>)}</nav> : null}

    {slug === "profil" ? <AccountSettings /> : null}

    {["messages", "notifications"].includes(slug) ? <section className={styles.panel}><div className={styles.sectionHead}><h2>Activité récente</h2><button onClick={load}>Actualiser</button></div>{visibleNotifications.length ? <div className={styles.notificationList}>{visibleNotifications.map((item) => <button type="button" key={item.id} onClick={() => openNotification(item)}><span><Icon name={item.read ? "mail" : "bell"} /></span><div><strong>{item.title ?? "Notification"}</strong><p>{item.message ?? ""}</p></div><small>{item.createdAt ? new Date(item.createdAt).toLocaleDateString("fr-FR") : ""}</small></button>)}</div> : <div className={styles.empty}><Icon name="mail" /><h3>Aucun élément</h3><p>Aucune notification ne correspond à ce filtre.</p></div>}</section> : null}
    ...
    {role === "formateur" && ["mediatheque", "ressources", "evaluations"].includes(slug) ? <section className={styles.panel}><h2>{details.title}</h2><p>Les vidéos et supports sont liés aux leçons de vos formations. Ouvrez une formation pour les gérer.</p><Link href={slug === "mediatheque" ? "/learning/formateur/studio" : "/learning/formateur/formations"} className={styles.primaryButton}>{slug === "mediatheque" ? "Ouvrir le studio" : "Gérer mes formations"}</Link></section> : null}
    {role === "formateur" && ["apprenants", "analytics"].includes(slug) ? <section className={styles.panel}><h2>{slug === "apprenants" ? "Inscriptions par formation" : "Activité de mes formations"}</h2>{courses.length ? <div className={styles.notificationList}>{courses.map((course) => <Link key={course.id ?? course.title} href={course.id ? `/learning/formateur/formations/${course.id}` : "/learning/formateur/formations"}><strong>{course.title}</strong> · {course.learners ?? 0} apprenant(s) · {course.lessonsCount ?? 0} leçon(s)</Link>)}</div> : <p>Aucune formation créée pour le moment.</p>}</section> : null}
  </>;
}
