...
export default function InstitutionPage() {
  const { context, loading, error: contextError, mismatch } = useCampusContext("direction");
  ...
  return <Shell role="direction" activeSlug="etablissement" displayName={context?.displayName} institutionName={context?.institutionName} note={contextError ?? null}>
    <header className={styles.head}><div><small>DIRECTION</small><h1>Mon établissement</h1><p>Informations publiques et coordonnées de l’établissement.</p></div><Link href="/campus/direction" className={styles.back}>← Accueil</Link></header>
