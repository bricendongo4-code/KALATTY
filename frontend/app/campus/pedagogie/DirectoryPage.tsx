...
export default function DirectoryPage({ section }: { section: "etudiants" | "enseignants" }) {
  ...
  return <Shell role="pedagogie" activeSlug={section} displayName={context?.displayName} institutionName={context?.institutionName} note={contextError ?? null}>
    <header className={styles.head}><div><small>RESPONSABLE PÉDAGOGIQUE</small><h1>{section === "etudiants" ? "Étudiants" : "Enseignants"}</h1><p>Annuaire de votre établissement.</p></div><Link href="/campus/pedagogie" className={styles.back}>← Accueil</Link></header>
